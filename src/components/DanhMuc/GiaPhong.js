...
export default class GiaPhong extends Component {
    ...
    giamdan() {
        fetch(`http://phatnguyen.nikinpham.com/be/public/index.php/api/giaphong/giamdan`)
    ...
    tangdan() {
        fetch(`http://phatnguyen.nikinpham.com/be/public/index.php/api/giaphong/tangdan`)
            .then(response => response.json())
            .then(data => {
                this.setState({
                    phongtro: data,
            ...
            <React.Fragment>
                ...
                {!isLoading ? (
                    phongtro.map(phongtros => {
                        const { TieuDe, MoTa, HinhAnh, Gia, TrangThai } = phongtros;
